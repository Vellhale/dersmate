...
$reg = Send Post '/api/auth/register' @{ email = $email; password = 'Demo12345'; displayName = "Pref $stamp"; termsVersion = (& "$PSScriptRoot\yasal-surum.ps1"); ageConfirmed = $true; hwidHash = $hwid } $null
Send Post '/api/auth/verify-email' @{ email = $email; code = $reg.verificationToken } $null | Out-Null
...
$reg2 = Send Post '/api/auth/register' @{ email = $email2; password = 'Demo12345'; displayName = "Pref2 $stamp"; termsVersion = (& "$PSScriptRoot\yasal-surum.ps1"); ageConfirmed = $true; hwidHash = $hwid2 } $null
Send Post '/api/auth/verify-email' @{ email = $email2; code = $reg2.verificationToken } $null | Out-Null
