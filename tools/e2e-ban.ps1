...
function NewUser($prefix, $hwid) {
    ...
    $reg = Api POST '/api/auth/register' @{ email = $email; password = 'Parola12345'; displayName = "$prefix Kullanici"; termsVersion = (& "$PSScriptRoot\yasal-surum.ps1"); ageConfirmed = $true }
...
$reg2 = Api POST '/api/auth/register' @{ email = $yeniHesap; password = 'Parola12345'; displayName = 'Kacak Deneme'; termsVersion = (& "$PSScriptRoot\yasal-surum.ps1"); ageConfirmed = $true }
Api POST '/api/auth/verify-email' @{ email = $yeniHesap; code = $reg2.verificationToken } | Out-Null
